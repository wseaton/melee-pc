...
| `MELEE_AUDIO_DUMP=<file>` | Also write the mix as raw f32 stereo 32 kHz. The mixer is normally driven by SDL's playback device, so the dump is wall-clock length. Under `MELEE_CAPTURE` the simulation drives the mixer instead, the dump is not written, and the audio goes into the recording. |
